...
struct GetMusicVideosCommand: AsyncParsableCommand, ToolRunnableCommand {
    static let configuration = CommandConfiguration(
        commandName: "get-music-videos",
        abstract: "Fetch catalog music videos by ids. Uses user storefront when available."
    )

    @OptionGroup var global: GlobalOptions

    @Option(name: .customLong("ids"), help: "Comma-separated music video IDs.")
    var ids: String
    ...
    func run() async throws {
        ...
        try await runTool(toolName: "get_music_videos", arguments: args)
    }
}
