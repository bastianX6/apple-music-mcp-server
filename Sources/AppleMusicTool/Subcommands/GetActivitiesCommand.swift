...
struct GetActivitiesCommand: AsyncParsableCommand, ToolRunnableCommand {
    static let configuration = CommandConfiguration(
        commandName: "get-activities",
        abstract: "Fetch catalog activities by ids. Uses user storefront when available."
    )

    @OptionGroup var global: GlobalOptions

    @Option(name: .customLong("ids"), help: "Comma-separated activity IDs.")
    var ids: String
    ...
    func run() async throws {
        ...
        try await runTool(toolName: "get_activities", arguments: args)
    }
}
