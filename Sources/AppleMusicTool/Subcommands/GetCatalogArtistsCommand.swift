...
struct GetCatalogArtistsCommand: AsyncParsableCommand, ToolRunnableCommand {
    static let configuration = CommandConfiguration(
        commandName: "get-catalog-artists",
        abstract: "Fetch catalog artists by ids. Uses user storefront when available."
    )

    @OptionGroup var global: GlobalOptions

    @Option(name: .customLong("ids"), help: "Comma-separated artist IDs.")
    var ids: String
    ...
    func run() async throws {
        ...
        try await runTool(toolName: "get_catalog_artists", arguments: args)
    }
}
