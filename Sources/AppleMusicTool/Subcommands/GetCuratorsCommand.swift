...
struct GetCuratorsCommand: AsyncParsableCommand, ToolRunnableCommand {
    static let configuration = CommandConfiguration(
        commandName: "get-curators",
        abstract: "Fetch catalog curators by ids. Uses user storefront when available."
    )

    @OptionGroup var global: GlobalOptions

    @Option(name: .customLong("ids"), help: "Comma-separated curator IDs.")
    var ids: String
    ...
    func run() async throws {
        ...
        try await runTool(toolName: "get_curators", arguments: args)
    }
}
