...
struct AddFavoritesCommand: AsyncParsableCommand, ToolRunnableCommand {
    static let configuration = CommandConfiguration(
        commandName: "add-favorites",
        abstract: "Add favorites (may return 405)."
    )
    ...
    @Option(name: .customLong("resource-type"), help: "Resource type when using --ids-csv (songs, albums, playlists, music-videos, stations).")
    ...
    func run() async throws {
        ...
        try await runTool(toolName: "add_favorites", arguments: args)
    }
}
